package com.hades.example.java._15_security.cryp.rsa;
...
public class RSATool {
    ...
    private PublicKey getPublicKey(String publicKeyBase64) {
        try {
            ...
            byte[] result  = Base64Tool.getInstance().decode(publicKeyBase64);
    ...
    private PrivateKey getPrivateKey(String privateKeyBase64) {
        try {
            KeyFactory keyFactory = KeyFactory.getInstance(ALGORITHM_NAME);
            byte[] result  = Base64Tool.getInstance().decode(privateKeyBase64);
...
//            Key privateKey = getSavedKey(PRIVATE_KEY_FILE_NAME);
            ...
            byte[] inputs = Base64Tool.getInstance().decode(toDecryptContent);;
            /** 执行解密操作 */
